...
El solver CBC reportó estado %{{ILP_STATUS}}%
\textit{\%\%ILP\_STATUS\%\%} con costo $Z^* = \%\%ILP\_COST\%\%$ y $|S^*| = \%\%ILP\_NSEL\%\%$ agrupaciones, en \%\%ILP\_TIME\%\%~s. La cota LP fue $z_{LP} = \$27{,}859.93$, confirmando un gap de integralidad del \%\%ILP\_GAP\%\%\%.

\subsection{Heurística \textit{greedy}}
La heurística de Chvátal seleccionó \%\%GREEDY\_NSEL\%\% agrupaciones con costo \%\%GREEDY\_COST\%\% en \%\%GREEDY\_TIME\%\%~s, una brecha de \%\%GREEDY\_GAP\%\%\% respecto al óptimo PLE.

\subsection{Algoritmo Genético (semilla principal)}
Con semilla 13, el AG alcanzó costo $Z_{GA} = \%\%GA13\_COST\%\%$ con \%\%GA13\_NSEL\%\% agrupaciones en \%\%GA13\_TIME\%\%~s, brecha del \%\%GA13\_GAP\%\%\% respecto al PLE.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/fig_convergencia_ga.png}}
...
\midrule
7  & \%\%R7\_C\%\%   & \%\%R7\_S\%\%   & \%\%R7\_G\%\%\%   \\
13 & \%\%R13\_C\%\%  & \%\%R13\_S\%\%  & \%\%R13\_G\%\%\%  \\
21 & \%\%R21\_C\%\%  & \%\%R21\_S\%\%  & \%\%R21\_G\%\%\%  \\
42 & \%\%R42\_C\%\%  & \%\%R42\_S\%\%  & \%\%R42\_G\%\%\%  \\
99 & \%\%R99\_C\%\%  & \%\%R99\_S\%\%  & \%\%R99\_G\%\%\%  \\
\midrule
\textbf{Media} & \%\%R\_MEAN\%\% & --- & --- \\
\textbf{Desv. estándar} & \%\%R\_STD\%\% & --- & --- \\
\textbf{Variabilidad relativa} & \%\%R\_CV\%\%\% & --- & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/fig_robustez.png}}
...
Greedy Chvátal     & \%\%GREEDY\_COST\%\% & \%\%GREEDY\_NSEL\%\% & $<0.01$        & \%\%GREEDY\_GAP\%\%\% \\
Algoritmo Genético & \%\%GA13\_COST\%\% & \%\%GA13\_NSEL\%\% & \%\%GA13\_TIME\%\% & \%\%GA13\_GAP\%\%\% \\
\textbf{ILP (CBC)} & \%\%ILP\_COST\%\%  & \%\%ILP\_NSEL\%\%  & \%\%ILP\_TIME\%\%  & \textbf{ref.} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/fig_comparativa_tiempos.png}}
...
Los resultados confirman la complementariedad de los enfoques exacto y metaheurístico. El método PLE es capaz de explorar un árbol de búsqueda extenso y, cuando el tiempo lo permite, garantizar optimalidad global. Sin embargo, el alto \textit{gap} de integralidad cercano al 44\% característico de esta instancia obliga al solver a una exploración profunda, lo que se refleja en tiempos cercanos al límite impuesto. Esta dificultad es coherente con la literatura especializada: instancias densas del SCP suelen presentar relajaciones lineales muy fraccionarias \cite{caprara1999}.

El AG, en contraste, entrega soluciones de calidad equivalente en una fracción del tiempo. El componente decisivo es el operador de reparación: sin él, las brechas observadas en la literatura para AG aplicados al SCP suelen oscilar entre 5\% y 10\% \cite{beasley1996}; con él, las brechas reportadas en este trabajo se sitúan consistentemente por debajo del 2\% en las cinco semillas evaluadas. El análisis de robustez con coeficiente de variación inferior al 1\% indica que el AG no depende críticamente del azar de la inicialización.

Desde la perspectiva aplicada al problema dermatológico, los resultados sugieren que la selección de subconjuntos de entrenamiento del HAM10000 mediante SCP es computacionalmente viable.

\section{Conclusiones}

\begin{itemize}
\item La complejidad NP-difícil del SCP es medible: la brecha de integralidad cercana al 44\% confirma alta dificultad combinatoria de la instancia.
\item El operador de reparación es el componente decisivo del AG: sin él, la brecha se sitúa entre 5\% y 10\%; con él, por debajo del 2\%.
\item El AG ofrece el mejor balance calidad-velocidad para aplicaciones reales con restricciones de tiempo.
\item La formulación SCP es directamente extensible al HAM10000 completo como trabajo futuro.
\end{itemize}
